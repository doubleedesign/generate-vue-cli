...
export default function initGenerateComponentCommand(args, cliConfigFile, program) {
	...
	// Static component command option defaults.
		...
		.option(
			'--type <type>',
			'You can pass a component type that you have configured in your GVC config file.',
			'default'
		)
		.option(
			'-d, --describe <describe>',
			'Describe the component you want GVC to generate (e.g., Create a counter component that increments by one when I click on the increment button).',
			null
		)
		.option(
			'-f, --flat',
